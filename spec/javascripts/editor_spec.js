describe("Editor", function() {
  ...
  it("generates the right number of sections", function() {
    expect(editor.sections.length).toBe(7);
    expect($('.inline-section').length).toBe(7);
    expect(editor.sections[0]).toBe('');
    expect(editor.editableSections.length).toBe(4);
    expect($('.inline-edit-form').length).toBe(4);
    expect($('.inline-edit-form textarea').length).toBe(4);
  });
